    ...
    public sealed class BasicMemory : MemoryProxy
    {
        ...
        public override bool AttachToProcess(string handle)
        {
            if (string.IsNullOrWhiteSpace(handle))
                throw new ArgumentException("Process handle cannot be null or empty", nameof(handle));

            // Accept names with or without .exe extension
            var procName = Path.GetFileNameWithoutExtension(handle);
